...
#
# It was wrong about both, and neither shape was in the table. Driven with
# `markdown_it.MarkdownIt("commonmark")` over every document of at most three
# lines on a twelve-token alphabet — 1884 of them — against the readers this
# repository shipped at `a90862b`:
#
#     1884 documents   unsound readings 35   refusals the renderer reads 20
...
def _renderer_reading(verdict: str | None) -> tuple[str, str] | None:
    """The renderer's verdict, read as this project reads a release heading.

    `None` in, `None` out — the renderer made no `<h2>`. A verdict that IS an
    `<h2>` but does not parse as `<version> — <rest>` is also `None`: our
    readers refuse it, and refusing is the same answer as having nothing to
    read as far as the relation below is concerned. The parse is
    :data:`_HEADING`, imported from the Python reader rather than re-spelled,
    because a second spelling of the heading grammar here would be exactly the
    duplication this corpus exists to remove.
    """
    if verdict is None:
        return None
    from test_changelog_names_the_version import _HEADING  # noqa: PLC0415

    parsed = _HEADING.match("## " + verdict.strip())
    return None if parsed is None else (parsed.group("version"),
                                        parsed.group("rest"))


def _python_reading(changelog: str) -> tuple[str, str] | None:
    """What the Python twin reads, with all three refusals folded to `None`."""
...
def test_BOTH_readers_of_the_newest_heading_agree(label, changelog, must_read,
    ...
    **CONSERVATIVE REFUSAL IS A PASS HERE AND THAT IS THE POINT.** Several
    ...
    holds that neither reader ever reads a heading the renderer does not.
    """
    reading = _renderer_reading(verdict)
    got = _python_reading(changelog)

    if must_read:
        assert reading is not None, (
            f"{label}: the corpus says this document must be READ, and the "
            f"renderer's verdict {verdict!r} does not parse as a release "
            f"heading, so there is nothing for the readers to read. The row "
            f"is self-contradictory — fix `must_read` or fix the document in "
            f"`tests/_changelog_renderer_corpus.py`."
        )
        assert got == reading, (
            f"{label}: the renderer's newest <h2> is {reading!r} and the "
            f"Python twin read {got!r}. This row is marked `must_read`, so a "
            f"refusal is a failure here too — being conservative about a "
            f"document this project has to be able to publish is not "
            f"conservative, it is a release that cannot happen."
        )
    else:
        assert got is None, (
            f"{label}: the corpus marks this document as one both readers "
            f"must REFUSE and the Python twin read {got!r} from it. The "
            f"renderer's own verdict is {verdict!r}."
        )
...
def test_the_two_readers_agree_over_the_whole_corpus(tmp_path):
    ...
    1884 documents at the commit that wrote this — every document of one to
    :data:`MAX_LINES` lines over `tests/_changelog_renderer_corpus.py`'s
    twelve-token alphabet, DERIVED here by the same function the generator
    used rather than stored, so the corpus cannot be re-labelled by a
    reordering. The count is read off the corpus rather than typed.
    ...
    * PYTHON, at full strength: for every document, if the twin reads a
      heading at all, it is the heading the RENDERER read. Refusing is always
      allowed — see the `must_read` rows above for where it is not.
    * BASH, by agreement: the gate has no channel for "the heading I read"
    ...
    line grammars, **35 of the 1884 are read as a heading the renderer does
    not put there, and the bash gate returns `rc=0` on all 35** — the two
    readers agreeing, with nothing to be right about. This branch's readers
    are 0 and 0 on the same corpus, and refuse 56 documents whose newest
    `<h2>` the renderer reads without difficulty, which is the trade taken
    deliberately.

    **WHAT THIS CANNOT SEE.** The alphabet. Twelve tokens of at most three
    lines is not the space of Markdown documents, and a construct not spelled
    by one of those tokens is not driven here — the block quotes, list items,
    tabs, form feeds and seven-hash lines are single rows in `NAMED` above and
    are not swept against each other. A wider sweep was taken by hand at this
    commit and is recorded at
    ...
    for text, code in zip(documents, _corpus.PRODUCT):
        reading = _renderer_reading(_corpus.VERDICTS[code])
        got = _python_reading(text)
        if got is not None:
            accepted += 1
            if got != reading:
                unsound.append((text, reading, got))
    ...
    assert not unsound, (
        ...
        f"twin as a heading the renderer does not put there. Each row is "
        f"(document, renderer, twin):\n"
        + "\n".join(f"  {row!r}" for row in unsound[:8])
    )
    assert not drift, (
...
def test_the_recorded_renderer_verdicts_are_still_the_renderers():
    ...
    **AND THE COLUMN DOES NOT MOVE BETWEEN THE TWO VERSIONS THAT WERE TRIED.**
    ...
    product, named, version = _corpus_tool.render(_corpus)

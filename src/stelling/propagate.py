...
class _Propagator:
    def __init__(
        ...
        # place the empty-value rule is stated. THREE CALLERS, all
        # real-mode only, each recording a fact it can actually establish:
        #
    ...
    def _strict_sign_out(self, eqn: ir.JaxprEqn, params, ins) -> int:
        ...
        path, :meth:`_record_strict_sign`, with three callers — a strict
        assume in :meth:`_classify_assumed_pred`, a CONSTVAR bind in
        :meth:`run`, and this function's result at the one call site in
        :meth:`eqn` — and all three are real-mode only. The constant
        ...
        **THE EMPTY-OUTPUT RULE IS NOT HERE.** "Every element is > 0" is
        ...
        the produced box's own size is in hand. The two OPERAND-side empty
        guards below (``reduce_sum``, ``dot_general``) are a different rule
        and stay: an empty sum has a size-1 output whose value is 0.
        ...
        **AND WHY THAT TOLERANCE IS NOT EXTENDED TO EVERY ROW.** The
        divergence above is a corner: it needs an operand near the
        underflow boundary. ``exp`` and ``pow`` diverge over a half-line
        (``math.exp(-800) == 0.0``), and ``sign``'s own transfer refuses
        the definite answer inside the subnormal band on purpose. Those
        three are in :data:`_SIGN_NO_RULE` with the measurement, not
        forgotten.
        """
        prim = eqn.primitive

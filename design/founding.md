...
- **Custom-derivative primal consistency — the second flagship.** JAX checks neither that a `custom_vjp`'s `fwd` nor that a `custom_jvp` rule's primal output equals `f` (both verified on jax 0.10.2: the lying primal propagates through `value_and_grad` and `jax.jvp` with no error, and the grad trace carries the rule's math — `f`'s is absent). Every plain call executes `f`; every differentiated path executes the rule; no existing tool checks the two agree, and a disagreement has no runtime symptom. Population, counted as rules rather than equations (`design/rule-provenance.md`): 8 distinct hand-written rules in the census corpus — 7 jvp + 1 vjp, all library-authored (equinox/lineax/optimistix, none JAX's own), one of which accounts for 100 of 117 call sites. `check_grads` does catch lying primals. Reshaped after reading all eight rules (2026-07-18, `design/registration-rules-and-capacity.md`): every concrete rule returns its own primal verbatim — the primal-agreement obligation has zero surface in this corpus — and the live flagship obligation is **tangent-region agreement** for the two lineax norm rules, whose tangents are intended to equal the true Jacobian away from a stated excluded set. Compile as `|rule_tangent − J_f·t| ≤ tol` over the region minus the Intentional set. Compile it as the `|f − rule_primal| ≤ tol` equivalence query (commitment 1). Corollary, stated once and inherited by everything: *everything inside a grad trace comes from the grad trace — primal properties included*; verify grad-context properties on `grad(f)`'s jaxpr, never by transferring a verdict from `f`'s. See `design/transparent-primitives.md`.
